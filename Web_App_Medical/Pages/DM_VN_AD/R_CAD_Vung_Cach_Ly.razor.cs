...
{
    public partial class R_CAD_Vung_Cach_Ly
    {
        DxDataGrid<CAD_Vung_Cach_Ly> _Grid_Vung_Cach_Ly;
        private IList<CAD_Vung_Cach_Ly> p_arrVung_Cach_Ly;
        CAD_Vung_Cach_Ly p_objVung_Cach_Ly;
        ...
        #region CRUD
        async void OnRowDeleting(CAD_Vung_Cach_Ly p_objItem)
        {

            bool isConfirmed = await IJS.InvokeAsync<bool>("confirm", "are you sure");
            if (isConfirmed)
            {
                _db_Vung_Cach_Ly.Delete_AD_Vung_Cach_Ly(p_objItem.Auto_ID, p_objItem.Last_Updated_By);
            }
            OnInitialized();
            await InvokeAsync(StateHasChanged);
        }
        void OnRowStarting(CAD_Vung_Cach_Ly p_objItem)
        {
            if (p_objItem == null)
            {
                p_objVung_Cach_Ly = new CAD_Vung_Cach_Ly();
                IsNewRow = true;
            }
            else
            {
                p_objVung_Cach_Ly = p_objItem;
                IsNewRow = false;
            }
        ...
        void HandleValiSubmit()
        {
            try
            {
                CAD_Vung_Cach_Ly v_objItem = p_objVung_Cach_Ly;
                if (IsNewRow == true)
                {
                    v_objItem.Last_Updated_By = "admin";
                    _db_Vung_Cach_Ly.Insert_AD_Vung_Cach_Ly(v_objItem);
                }
                else
                {
                    v_objItem.Last_Updated_By = "admin";
                    _db_Vung_Cach_Ly.Update_AD_Vung_Cach_Ly(v_objItem);
                }
                _Grid_Vung_Cach_Ly.CancelRowEdit();
                LoadData();
                Error = "";
            }
            catch (Exception ex)
        ...
        private void Import_Excel()
        {
            ...
            try
            {
                ...
                {
                    CAD_Vung_Cach_Ly v_objItem = new CAD_Vung_Cach_Ly();
                    v_objItem.Ten_Vung = CUtility.Convert_To_String(v_row[0]);
                    v_objItem.Lat = CUtility.Convert_To_String(v_row[1]);
                    v_objItem.Lng = CUtility.Convert_To_String(v_row[2]);
                    v_objItem.Last_Updated_By = "admin";
                    _db_Vung_Cach_Ly.Insert_AD_Vung_Cach_Ly(v_conn, v_trans, v_objItem);
                }
                v_trans.Commit();
                p_strPath = "";
